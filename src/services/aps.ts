...
import {IAuthResponse} from "../type/auth.type";
...
export async function getInternalToken(): Promise<IAuthResponse> {
    console.log('Fetching/Refreshing internal token...');
    const token : IAuthResponse = await authClient.getTwoLeggedToken(
        config.aps.clientId,
    ...
    console.log('Internal token obtained/refreshed.');
    return token
}
